...
def load_and_index_files(repo_path):
    extensions = ['txt', 'md', 'markdown', 'rst', 'py', 'c', 'cpp', 'cs', 'go', 'rb', 'php', 'scala', 'html', 'htm',
                  'yaml', 'ini', 'toml', 'cfg', 'conf', 'sh', 'bash', 'sql', 'gitignore', 'dockerignore',
                  'editorconfig', 'ipynb']

    unstructured = ['js', 'java', 'xml', 'json', 'yml', 'css', 'scss']
    ...
    for ext in extensions:
        ...
        try:
            ...
            for item in unstructured:
                if ext.__contains__(item):
                    loader = UnstructuredFileLoader(str(repo_path), strategy="fast", mode="elements")
...
def search_documents(query, index, documents, n_results=5):
    ...
    tfidf_vectorizer = TfidfVectorizer(tokenizer=clean_and_tokenize, lowercase=True, stop_words='english', use_idf=True,
